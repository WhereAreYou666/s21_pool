...
double det(double **matrix, int n, int m) {
    ...
    if (n == 1) {
        ...
        for (int i = 0; i < n - 1; i++) result[i] = malloc(m * sizeof(double));
        for (int j = 0; j < n; j++) {
...
void invert(double **matrix, int n, int m) {
    ...
    for (int i = 0; i < n; i++) free(result[i]);
    free(result);
    for (int i = 0; i < n; i++) free(inv_matrix[i]);
    free(inv_matrix);
}